            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 h-full items-stretch">
                ...
                <div class="py-16 lg:py-0 text-center lg:text-left self-center">
                    ...
                    <div class="flex flex-col sm:flex-row gap-5 justify-center lg:justify-start">
                        <a href="{{ route('products') }}" class="inline-flex justify-center items-center bg-white text-black px-9 py-4 rounded-full font-bold text-base hover:bg-gray-200 transition-all duration-300 transform hover:scale-105 shadow-lg">
                            Explore Collection
                        </a>
                        <a href="{{ route('articles.index') }}" class="inline-flex justify-center items-center px-9 py-4 rounded-full font-bold text-base border border-gray-600 text-white hover:border-white hover:bg-white/10 transition-all duration-300">
                            Discover More
                        </a>
                    </div>
                </div>
                ...
                <div class="relative w-full h-full min-h-[400px] lg:min-h-full flex items-end justify-center lg:justify-end pb-0">
                    ...
                    <div class="relative w-full max-w-[550px] flex items-end justify-center">
                        ...
                        <div class="absolute top-[10%] right-0 md:-right-4 bg-white p-4 pr-6 rounded-2xl shadow-xl flex items-center gap-4 z-20 animate-bounce-slow">
                            <div class="w-12 h-12 bg-black rounded-xl flex items-center justify-center text-white shrink-0">
                        ...
                        <div class="absolute bottom-12 left-4 md:-left-8 bg-white p-5 rounded-[1.8rem] shadow-xl text-center z-20 animate-bounce-slow w-36" style="animation-delay: 2s;">
                            <div class="w-12 h-12 bg-black rounded-full flex items-center justify-center text-white mx-auto mb-2">
    ...
    <div class="relative">
        <div class="flex items-center overflow-hidden">
            ...
            <div class="flex items-center space-x-24 animate-scroll">
                @for ($i = 0; $i < 10; $i++)
                    <span class="text-3xl md:text-4xl font-bold tracking-widest uppercase text-black opacity-80 hover:opacity-100 transition-opacity duration-300 whitespace-nowrap leading-none">
                        THE PARANOIA
                    </span>
                @endfor
            </div>
...
<style>
    ...
    .flex:hover .animate-scroll { animation-play-state: paused; }
</style>
                ...
                <div class="relative rounded-3xl shadow-md cursor-pointer overflow-hidden group w-full"
                    ...
                    <div class="absolute inset-0 flex items-center justify-center p-8">
                        <div class="text-center">
                            ...
                            <span class="bg-white font-bold text-black px-6 py-2 rounded-full text-sm hover:bg-gray-200 transition-colors duration-200">
                                View More
                            </span>
                        </div>
                    </div>
...
<section class="w-full">
    <div class="min-h-80 flex items-center justify-center bg-[#1A1A1D]">
        <div class="text-center text-white px-4">
            ...
            <a href="{{ route('products') }}" class="bg-white text-black px-8 py-4 rounded-[2vw] font-semibold hover:bg-gray-200 transition-colors duration-200">
                Get Started
            </a>
        </div>
    </div>
...
<section id="products" class="py-12 md:py-16 bg-gray-50">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3 md:gap-4">
            @php
                $categoryIcons = [
                    't-shirt' => 'fa-tshirt', 'tshirt' => 'fa-tshirt', 'kaos' => 'fa-tshirt', 'shirt' => 'fa-tshirt',
                    'jacket' => 'fa-vest', 'jaket' => 'fa-vest', 'hoodie' => 'fa-vest',
                    'pants' => 'fa-socks', 'celana' => 'fa-socks', 'jeans' => 'fa-socks',
                    'hat' => 'fa-hat-cowboy', 'topi' => 'fa-hat-cowboy', 'cap' => 'fa-hat-cowboy',
                    'bag' => 'fa-shopping-bag', 'tas' => 'fa-shopping-bag',
                    'accessories' => 'fa-gem', 'aksesoris' => 'fa-gem',
                    'shoes' => 'fa-shoe-prints', 'sepatu' => 'fa-shoe-prints',
                ];
...
<section class="pt-8 pb-8 bg-gray-50" x-data="productFilter()">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="flex flex-wrap justify-center gap-2 sm:gap-4 md:gap-6 lg:gap-10 rounded-2xl md:rounded-3xl bg-[#1A1A1D] p-2 md:p-3">
            ...
            <button @click="filterByCategory('{{ $category->id }}')"
               :class="activeCategory === '{{ $category->id }}' ? 'bg-white text-black' : 'text-white hover:bg-white/20'"
               class="px-4 sm:px-6 md:px-8 py-2 md:py-3 rounded-full text-sm md:text-base font-semibold transition-all duration-300 transform">
                {{ $category->name }}
            </button>
            @endforeach
            ...
            <template x-for="product in filteredProducts" :key="product.id">
                <div class="group bg-white rounded-xl md:rounded-2xl shadow-md hover:shadow-xl transition-all duration-300 overflow-hidden">
                    ...
                    <div class="p-3 md:p-4">
                        <div class="mb-2">
                            <span class="text-xs font-medium text-gray-600 bg-gray-100 px-1.5 py-0.5 rounded" x-text="product.category_name || 'Product'"></span>
                        </div>

                        <h3 class="text-sm md:text-base font-bold text-black mb-2 line-clamp-2" x-text="product.name"></h3>

                        <p class="text-base md:text-lg font-black text-black mb-3" x-text="'Rp ' + product.price_formatted"></p>

                        <a :href="'/products/' + product.slug"
                           class="block w-full bg-black text-white font-semibold py-2 md:py-2.5 rounded-lg hover:bg-gray-800 active:scale-95 transition-all duration-200 shadow-md hover:shadow-lg text-center text-xs md:text-sm">
                            View Details
                        </a>
                    </div>
                </div>
...
<section class="py-12 md:py-16 bg-gray-50">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8 md:mb-12">
            ...
            <a href="{{ route('rewards') }}" class="bg-black text-white px-4 md:px-6 py-2 md:py-3 rounded-full font-semibold hover:bg-gray-800 transition duration-300 transform hover:scale-105 shadow-lg text-sm md:text-base">
                View All
            </a>
        </div>
        ...
        <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-4 gap-3 md:gap-4 lg:gap-5">
            ...
            <div class="group bg-white rounded-xl md:rounded-2xl shadow-md hover:shadow-xl transition-all duration-300 overflow-hidden">
                <div class="relative overflow-hidden">
                    ...
                    <div class="absolute top-2 left-2 bg-black text-white font-bold text-xs px-2 py-1 rounded-full">
                ...
                <div class="p-3 md:p-4">
                    <div class="mb-2">
                        <span class="text-xs font-medium text-black bg-gray-200 px-1.5 py-0.5 rounded">
                            {{ $product->category->name ?? 'Reward' }}
                        </span>
                    </div>

                    <h3 class="text-sm md:text-base font-bold text-black mb-2 line-clamp-2">
                        {{ $product->name }}
                    </h3>

                    <p class="text-base md:text-lg font-black text-black mb-3">
...
<section id="style-inspiration" class="py-12 md:py-16 bg-gray-50">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8 md:mb-12">
            ...
            <a href="{{ route('products') }}" class="bg-black text-white px-6 py-2.5 md:py-3 rounded-full font-semibold hover:bg-gray-800 transition duration-300 transform hover:scale-105 shadow-lg text-sm md:text-base">
                Explore All
            </a>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
            <a href="{{ route('products') }}" class="group relative overflow-hidden rounded-2xl md:rounded-3xl shadow-lg aspect-[4/5]">
                ...
                <div class="absolute bottom-0 left-0 right-0 p-4 md:p-6">
                    <span class="inline-block bg-white text-black text-xs font-bold px-3 py-1 rounded-full mb-2 md:mb-3">TRENDING</span>
            ...
            <a href="{{ route('products') }}" class="group relative overflow-hidden rounded-2xl md:rounded-3xl shadow-lg aspect-[4/5]">
                ...
                <div class="absolute inset-0 bg-gradient-to-t from-black/70 via-black/20 to-transparent"></div>
                <div class="absolute bottom-0 left-0 right-0 p-4 md:p-6">
                    <span class="inline-block bg-black text-white text-xs font-bold px-3 py-1 rounded-full mb-2 md:mb-3">NEW ARRIVAL</span>
