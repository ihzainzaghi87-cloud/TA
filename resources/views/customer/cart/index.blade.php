...
<style>
    ...
    .cart-hero-gradient { background: linear-gradient(90deg, #9333ea, #ec4899, #ef4444); }
    .cart-empty-icon { background: linear-gradient(135deg, #ede9fe, #fdf2f8); }
    .cart-placeholder-img { background: linear-gradient(135deg, #f3f4f6, #e5e7eb); }
    .cart-primary-gradient { background: linear-gradient(90deg, #9333ea, #ec4899); }
    .cart-subtle-gradient { background: linear-gradient(135deg, #ede9fe, #f9a8d4); }
</style>
@endpush
...
<section class="cart-hero-gradient text-white py-16">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
        ...
        <p class="mt-4 text-white/80 max-w-2xl mx-auto">Update quantities, remove items, or convert your purchases into reward points before checking out.</p>
    </div>
</section>
    ...
    @if($cartItems->isEmpty())
        <div class="bg-white rounded-3xl shadow-xl p-12 text-center">
            <div class="flex flex-col items-center space-y-6">
                ...
                <p class="text-gray-600 max-w-md">Jelajahi katalog kami lalu tambahkan produk favorit Anda. Semua barang yang dimasukkan akan tampil di sini.</p>
                <a href="{{ route('home') }}" class="inline-flex items-center gap-2 cart-primary-gradient text-white px-6 py-3 rounded-full font-semibold shadow-lg hover:shadow-2xl transform hover:-translate-y-0.5 transition">
                ...
                <div class="lg:col-span-2 space-y-6">
                    ...
                    <div class="bg-white rounded-2xl shadow-lg border border-gray-100 p-4 flex items-center justify-between">
                        <label class="inline-flex items-center gap-3">
                            <input type="checkbox" id="select-all" class="w-5 h-5 rounded border-gray-300 text-purple-600 focus:ring-purple-500">
                            <span class="text-sm font-semibold text-gray-700">Pilih Semua Produk</span>
                        </label>
                        <span class="text-xs text-gray-500">Centang produk yang ingin dihitung dan di-checkout</span>
                    </div>
                            ...
                            <div class="flex flex-col md:flex-row gap-6">
                                ...
                                <div class="flex md:flex-col items-start gap-3 md:w-32 w-full">
                                    ...
                                    <div class="flex-1 md:w-full">
                                        @if($productImage)
                                            <img src="{{ $productImage }}" alt="{{ $productName }}" class="w-full h-32 object-cover rounded-xl">
                                        @else
                                ...
                                <div class="flex-1 space-y-3">
                                    <div class="flex items-start justify-between gap-4">
                                        <div>
                                            ...
                                            <p class="text-sm text-gray-500 flex flex-wrap gap-2 mt-1">
                                                @if($variation)
                                                    @if($variation->color)
                                                        <span class="inline-flex items-center gap-1"><i class="fas fa-palette text-xs text-purple-500"></i>{{ ucfirst($variation->color) }}</span>
                                                    @endif
                                                    @if($variation->size)
                                                        <span class="inline-flex items-center gap-1"><i class="fas fa-ruler text-xs text-purple-500"></i>{{ strtoupper($variation->size) }}</span>
                                                    @endif
                                                    <span class="inline-flex items-center gap-1"><i class="fas fa-boxes text-xs text-purple-500"></i>Stok: {{ $variation->stock ?? 0 }}</span>
                                                @else
                                        ...
                                        <form action="{{ route('cart.destroy', $item->id) }}" method="POST" onsubmit="return confirm('Hapus item ini dari keranjang?');">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="text-gray-400 hover:text-red-500 transition" aria-label="Remove item"><i class="fas fa-trash"></i></button>
                                        </form>
                                    </div>
                                    ...
                                    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                                        <div>
                                            ...
                                            @if($unitPointPrice)
                                                <p class="text-sm text-amber-600"><i class="fas fa-star text-xs"></i>{{ number_format($unitPointPrice, 0, ',', '.') }} poin</p>
                                            @endif
                                        </div>
                                        ...
                                        <div class="flex items-center gap-3">
                                            <form action="{{ route('cart.update', $item->id) }}" method="POST" class="flex items-center gap-2">
                                                @csrf
                                                @method('PUT')
                                                <button type="button" class="w-8 h-8 rounded-full border border-gray-300 flex items-center justify-center text-gray-600 hover:border-purple-500 hover:text-purple-600" data-quantity-decrease><span class="sr-only">Kurangi</span>-</button>
                                                <input type="number" name="quantity" value="{{ $item->quantity }}" min="1" class="w-16 text-center border border-gray-200 rounded-lg py-1 focus:ring-2 focus:ring-purple-500 focus:border-purple-500" data-quantity-input>
                                                <button type="button" class="w-8 h-8 rounded-full border border-gray-300 flex items-center justify-center text-gray-600 hover:border-purple-500 hover:text-purple-600" data-quantity-increase><span class="sr-only">Tambah</span>+</button>
                                        ...
                                        <div class="text-right">
                                            ...
                                            @if($linePointPrice)
                                                <p class="text-sm text-amber-600"><i class="fas fa-star text-xs"></i>{{ number_format($linePointPrice, 0, ',', '.') }} poin</p>
                                            @endif
                                        </div>
                                    </div>
                                </div>
                ...
                <aside class="lg:col-span-1">
                    <div class="bg-white rounded-3xl shadow-xl border border-gray-100 p-8 space-y-6">
                        <div>
                            ...
                            <p class="text-xs text-gray-400 mt-1">Total di bawah mengikuti produk yang Anda centang.</p>
                        </div>

                        <div class="space-y-4">
                            <div class="flex items-center justify-between text-gray-600">
                                <span>Total Harga</span>
                                <span class="font-semibold text-gray-900" id="selected-total-price">Rp 0</span>
                            </div>
                            <div class="flex items-center justify-between text-amber-600">
                                <span>Total Poin</span>
                                <span class="font-semibold" id="selected-total-point">0 poin</span>
                            </div>
                        </div>
                        ...
                        <div class="space-y-3">
                            ...
                            <button id="checkout-btn" type="button" class="w-full flex items-center justify-center gap-2 cart-primary-gradient text-white font-semibold py-3 rounded-full shadow-lg hover:shadow-2xl transform hover:-translate-y-0.5 transition">
