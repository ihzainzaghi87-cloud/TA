...
<div class="space-y-6">
    ...
    <div class="bg-white dark:bg-gray-800 shadow rounded-xl border border-gray-200 dark:border-gray-700 overflow-hidden">
        ...
        <form action="{{ route('admin.banners.update', $banner) }}" method="POST" enctype="multipart/form-data" class="p-6 space-y-6">
            ...
            <div>
                <label for="image_mobile" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                    Change Banner Image (Mobile) <span class="text-gray-400 text-xs">(Optional)</span>
                </label>
                <div class="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-gray-300 dark:border-gray-600 border-dashed rounded-lg hover:border-amber-400 dark:hover:border-amber-500 transition-colors duration-200 bg-gray-50 dark:bg-gray-900/40">
                    <div class="space-y-1 text-center">
                        <svg class="mx-auto h-12 w-12 text-gray-400" stroke="currentColor" fill="none" viewBox="0 0 48 48">
                        ...
                        <div class="flex text-sm text-gray-600 dark:text-gray-400">
                            <label for="image_mobile" class="relative cursor-pointer bg-white dark:bg-gray-800 rounded-md font-medium text-amber-600 hover:text-amber-500">
                                <span>Upload a new file</span>
                                <input id="image_mobile" name="image_mobile" type="file" class="sr-only" accept="image/*" onchange="previewImageMobile(event)">
                            </label>
                            <p class="pl-1">or drag and drop</p>
                        </div>
                        <p class="text-xs text-gray-500">PNG, JPG, GIF, WEBP up to 2MB</p>
                ...
                <div id="image-mobile-preview" class="mt-4 hidden">
                    <div class="relative inline-block">
                        <img src="" alt="Mobile Preview" class="max-h-64 rounded-lg border-2 border-gray-200 dark:border-gray-600">
                        <button type="button" onclick="removePreviewMobile()" class="absolute top-2 right-2 bg-red-600 text-white rounded-full p-1 hover:bg-red-700">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path>
                            </svg>
                        </button>
...
function removePreviewMobile() {
    const preview = document.getElementById('image-mobile-preview');
    const input = document.getElementById('image_mobile');
    preview.classList.add('hidden');
    input.value = '';
}
</script>
